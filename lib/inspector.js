...
function inspectHTML(obj, res, cb) {
	...
	var tags = {};
	var priorities = {};
	var curText = null, curKey = null, curSchemaType = null, curSchemaLevel, curLevel = 0;
	var mainSchemaType;
	...
	parser.on('startTag', function(name, atts, selfClosing) {
		...
		for (i=0; i < atts.length; i++) {
			...
			if (att.name == "itemtype") {
				// if no type other than WebPage was found before, consider
				// the embedded object is the main type
				if (mainSchemaType) continue;
				...
				debug("detected embedded Object", val, curSchemaType);
				curSchemaType = val;
		...
		var priority = selectors[name] && selectors[name].priority || 0;
