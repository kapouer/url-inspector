...
var inspectors = {
	...
	file: [inspectFile, 10000],
	archive: [inspectArchive, 10000]
};
...
function feedExiftool(res, cb) {
	var exif = ChildProcess.spawn('exiftool', ['-b', '-j', '-fast', '-']);
	exif.stderr.pipe(BufferList(function(err, data) {
		if (err) return cb(err);
		if (data && data.length) return cb(data.toString());
	}));
	exif.stdout.pipe(BufferList(function(err, data) {
		if (err) return cb(err);
		var tags;
		try {
...
function inspectHTML(obj, res, cb) {
	...
	var tags = {};
	var curText = null, curKey = null, curSchemaType = null;
	// node nesting level - know if itemprop is the child of the itemscope with
	// the right itemtype
	var current = 0, previous = 0;

	parser.on('startTag', function(name, atts, selfClosing) {
		current++;
		var i, att, attmap, valmap, key, name = name.toLowerCase(), val, attsObj = {};
		for (i=0; i < atts.length; i++) {
			...
			if (att.name == "itemtype") {
				if (val.endsWith('/VideoObject')) val = 'video';
				else if (val.endsWith('/AudioObject')) val = 'audio';
				else if (val.endsWith('/ImageObject')) val = 'image';
				else if (val.endsWith('/WebPage')) val = 'link';
				else continue;
				curSchemaType = val;
				previous = current;
				if (!tags.type || tags.type == "link") tags.type = val;
				continue;
			} else if (att.name == "itemprop") {
				if (!curSchemaType || previous != current - 1) continue;
		...
		if (mkey && attsObj[mkey]) tags[key] = attsObj[mkey];
	});
	...
	parser.on('endTag', function(name) {
		...
		if (curText != null) tags[curKey] = curText;
		curText = null;
		curKey = null;
	});
	...
	function finish() {
		...
		if (type) {
			...
			else if (/^(audio|music)/i) type = 'audio';
			else if (/^(image|photo)/i) type = 'image';
			else type = null;
		...
		// because that's we're going to get in the embed response
		var fetchEmbed = !obj.noembed && obj.oembed && (!obj.thumbnail || !obj.embed);
		delete obj.noembed;
		if (fetchEmbed && false) {
			obj.type = "embed";
...
function inspectMedia(obj, res, cb) {
	feedExiftool(res, function(err, tags) {
		if (err) return cb(err);
		importTags(tags, obj, {
			imagewidth: 'width',
			imageheight: 'height',
		...
		if (!obj.thumbnail && tags.Picture && tags.PictureMIMEType) {
			obj.thumbnail = `data:${tags.PictureMIMEType};` + tags.Picture.replace(/^base64:/, 'base64,');
		}
