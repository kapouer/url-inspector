...
function inspector(url, opts, cb) {
	...
	request(oEmbedUrl.obj || urlObj, obj, function(err, obj, tags) {
		if (typeof oEmbedUrl.builder == "function") {
			oEmbedUrl.builder(urlObj, obj);
		}
		if (err) {
			if (oEmbedUrl.obj) {
...
function normalizeAfter(cb) {
	return function (err, obj) {
		cb(err, normalize(obj));
	};
}
...
function importJsonLD(tags, text) {
	try {
