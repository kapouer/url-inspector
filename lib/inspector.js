...
function inspector(url, opts, cb) {
	...
	request(oEmbedUrl.obj || urlObj, obj, function(err, obj, tags) {
		...
		normalize(obj);
		if (!opts.nosource && obj.source && obj.mime == "text/html" && obj.source != obj.url && /video|audio|image/.test(obj.type)) {
			// let's inspect this source
			var realcb = cb;
			cb = function(err, obj) {
				if (err) return realcb(err, obj);
				...
				inspector(obj.source, opts, function(err, sourceObj) {
					if (err) {
						...
						return realcb(null, obj);
					}
					['url', 'mime', 'ext', 'type', 'size', 'width', 'height'].forEach(function(key) {
